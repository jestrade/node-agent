...
router.post("/webhook", async (req, res) => {
  ...
  try {
    if (type === "user.created") {
      ...
      const msg = await generateMessage({ purpose: "Ayudar a publicar la primera propiedad", context: { user: u } });
    ...
    if (type === "listing.created") {
      ...
      if (!payload.description || (payload.photos?.length || 0) < 3) {
        ...
        const msg = await generateMessage({ purpose: "Sugerir mejorar descripción y fotos", context: { user: owner, listing: l }});
    ...
    if (type === "listing.view") {
      ...
      if (l.visits >= 50 && l.visits % 50 === 0) {
        ...
        const msg = await generateMessage({ purpose: "Sugerir upgrade a Pro por buena tracción", context: { user: owner, listing: l }});
        await sendEmail(owner.email, msg.subject, `${msg.body}\n\n${msg.cta}`);
